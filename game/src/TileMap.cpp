...
void TileMap::LoadList(const std::string& file) {
    ...
    for (auto it = j["files"].begin(); it != j["files"].end(); ++it) {
        TileMapsFiles.push_back(ASSETS_PATH("/map/") +
...
void TileMap::Load(const std::string& file) {
    ...
    tileSet = new TileSet(ASSETS_PATH("/map/") + tileSetFile);
...
void SpawnDust(Vec2 pos) {
    ...
    go->AddComponent(new Sprite(*go, ASSETS_PATH("/img/dust.png"), 3, 0.2, 3 * 0.2));
